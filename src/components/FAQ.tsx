...
export default function FAQ() {
  return (
    <section id="faq" className="relative px-5 pb-20 md:pb-96 min-h-screen bg-transparent z-10">
      ...
      <div className="absolute top-20 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-[700px] z-0 pointer-events-none mix-blend-screen"
        style={{
        ...
        <div className="border-t border-white/10 relative z-10">
          {faqs.map((faq, index) => (
            <Reveal key={index} delay={index * 0.1}>
              <div className="border-b border-white/10 py-7 md:py-12">
                <div className="flex flex-col gap-3 md:hidden">
                  <div className="flex items-start gap-3">
                    <div
                      className={`flex-shrink-0 w-7 h-7 flex items-center justify-center text-[10px] font-bold mt-0.5 ${
                        faq.purple
                    ...
                    <h3
                      className={`text-[15px] font-medium leading-tight ${
                        faq.purple ? 'text-primary' : 'text-white'
                      }`}
                    >
                      {faq.question}
                    </h3>
