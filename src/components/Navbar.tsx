  ...
  return (
    <nav className="fixed top-4 left-3 right-3 z-50 flex flex-col items-center pointer-events-none">
      <div className="w-full max-w-3xl pointer-events-auto">
        <div
          className={`flex items-center justify-between rounded-full px-4 py-2 md:px-6 md:py-2 bg-white border border-white/20 shadow-xl transition-shadow duration-300 ${
        ...
        >
          {/* Logo */}
          <a href="#hero" className="flex-shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary rounded-full">
            <img
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navLinks.map((link) => (
              <a
                key={link.href}
                href={link.href}
                className="text-sm font-medium text-zinc-800 hover:text-primary transition-colors duration-200 focus:outline-none focus-visible:text-primary"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-1">
            <button
              className="hidden md:inline-block px-3 py-1.5 md:px-6 md:py-2 bg-primary text-white rounded-full text-xs md:text-sm font-bold shadow-[0_4px_6px_rgba(124,58,237,0.2)] hover:bg-primary-deep transition-colors duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-light/50"
              aria-label="Sign In"
            >
              Sign In
            </button>
